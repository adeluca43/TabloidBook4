...
import { createSubscription, deleteSubscription, getAllSubscriptions } from "../../managers/subscriptionManager";
...
export default function UserProfileDetails({loggedInUser}) {
 ...
 getAllSubscriptions().then(res => {
  setAllSubs(res)
  const subs = res.filter(s => s.authorId == id)
    setSubs(subs)
  }), [id])

  const handleSub = () => {
      const isSub = allSubs.find(sub => sub.subscriberId == loggedInUser.id && sub.authorId == id);
     if (isSub) {
